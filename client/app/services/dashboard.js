...
function transformRecentResponse(data) {
  ...
  const userRecentDashboards = _.filter(allUserDashboards, dashboard =>
    recentDashboardsIds.includes(dashboard.id.toString())
  );
  const sortedUserRecentDashboards = userRecentDashboards.sort((dashboardA, dashboardB) => {
    const priorityOfDashboardA = _.find(parsedRecentDashboards, dashboard => dashboard.id === dashboardA.id.toString())
      .priority;
    const priorityOfDashboardB = _.find(parsedRecentDashboards, dashboard => dashboard.id === dashboardB.id.toString())
      .priority;
